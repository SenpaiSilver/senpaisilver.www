<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>SenpaiSilver: Are you not entertained?</title>

    <meta name="description" content="Are you not entertained?"/>
    <meta name="keywords" content="SenpaiSilver, Senpai, Silver, gaming, blog, code, tweets"/>
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <style>
        @import url('https://fonts.googleapis.com/css?family=Lato|Major+Mono+Display|Merriweather|Montserrat|Noto+Serif+SC|Raleway|Roboto|Roboto+Condensed|Roboto+Mono|Sarabun|Source+Code+Pro');

        @font-face {
            font-family: 'senpaisilver-icons';
            src:  url('<?=base_url("assets/fonts/senpaisilver-icons.eot?xevsmz")?>');
            src:  url('<?=base_url("assets/fonts/senpaisilver-icons.eot?xevsmz#iefix")?>') format('embedded-opentype'),
                url('<?=base_url("assets/fonts/senpaisilver-icons.ttf?xevsmz")?>') format('truetype'),
                url('<?=base_url("assets/fonts/senpaisilver-icons.woff?xevsmz")?>') format('woff'),
                url('<?=base_url("assets/fonts/senpaisilver-icons.svg?xevsmz#icomoon")?>') format('svg');
            font-weight: normal;
            font-style: normal;
        }
    </style>

    <link href="/assets/dist/css/main.css" rel="stylesheet"/>
</head>
<body>
<div id="particles-js"></div>

<nav>
    <div class="container">
        <a href="<?=base_url()?>" class="home">
            <!-- <span class="icon home"></span> SenpaiSilver -->
            SenpaiSilver
        </a>
        <?php $this->load->view("base/menu"); ?>
    </div>
</nav>

<main>